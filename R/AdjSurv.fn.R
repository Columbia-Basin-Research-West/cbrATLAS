...
#' @param num.bootstrap.in (default = 1000) Number of resample iterations to estimate additional variance on survival estimates
...
AdjSurv.fn=function(taghist.file, taghist.format="atlas",taglife.file=NULL, taglife.model=NULL, num.release=1,
                    num.bootstrap.in = 250, adjust.cjs = T, rounding = 4, plot.taglife = T, study.type="single") {
  # helper fns
  ...
  # reformat detection file if in Program ATLAS format from ATLAS to flat
  ...
  if(study.type=="single"){
    for(rel.i in 1:length(rel.groups)){
      ...
      if(!is.null(taglife.file)){
        ...
        #	  print("num.bootstrap.in")
        #	  print(num.bootstrap.in)

        tt.rel2site=harmonic.tt2site.fn(data.in,num.period,site.names)
        ...
        ### once tag-life model has been selected, rerun to get bootstrapped se's on P(Li)
        mean.tag.p=cjs.taglife.corr(activetime.matrix=tt.rel2site$activetime.matrix,site.names=site.names,
                                    num.period=num.period,taglife.fit=taglife.fit,num.bootstrap=num.bootstrap.in,cjs.est=unadj.cjs.params$cjs.param)
    ...
    if(!is.null(taglife.file)){
      ...
      for(rel.i in 1:2){
        ...
        mean.tag.p[[rel.i]]=cjs.taglife.corr(activetime.matrix=tt.rel2site$activetime.matrix,
                                             ...
                                             num.period=num.period,
                                             taglife.fit=taglife.fit,
                                             num.bootstrap=num.bootstrap.in,
