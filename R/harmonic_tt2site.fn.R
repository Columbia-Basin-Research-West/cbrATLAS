...
#' @export
#'
harmonic.tt2site.fn=function(data.in,num.period,site.names){
  harmonic.tt = data.frame(matrix(0, nrow = num.period, ncol = 2))
  harmonic.tt[, 1] = site.names
  names(harmonic.tt) = c("Release to", "Travel Time (s.e.) num.tags")
  ...
  for (i in 1:num.period) {
	...
	harmonic.tt[i, 2] = harmonic(temp.time)
  }
  ...
  return(list(harmonic.tt=harmonic.tt,tt.matrix=tt.matrix,activetime.matrix=active.matrix))
}
